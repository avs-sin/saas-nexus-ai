...
interface ProductionAIAssistantProps {
  isOpen: boolean;
  onClose: () => void;
  data: ProductionDashboardData;
}

export function ProductionAIAssistant({ isOpen, onClose, data }: ProductionAIAssistantProps) {
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "model",
      text: "Hello! I'm your Production AI assistant. I can help with material requirements, work order scheduling, inventory questions, and production planning. What would you like to know?",
  ...
  const chatAction = useAction(api.nexusProductionActions.chatWithProductionAIAction);
      ...
      <Card className="h-full flex flex-col">
        <CardHeader className="bg-primary text-primary-foreground rounded-t-lg py-3 flex flex-row items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-primary-foreground text-base">Production Copilot</CardTitle>
          </div>
          <Button
            variant="ghost"
            size="sm"
        ...
        <div className="p-3 border-t bg-card rounded-b-lg">
          <div className="flex gap-2">
            <Input
              ...
              placeholder="Ask about materials, WOs..."
              className="flex-1"
            />
